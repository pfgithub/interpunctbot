...
router.add("wr", [], async(cmd, info) => { // wr <category> gets the wr for the specified category||default
	...
	if(categoryName) {
		...
		const categoryFilter = categories.items.filter(cat => cat.name.toLowerCase() === categoryName.toLowerCase());
	...
	const gameData = await sr.leaderboards(gameID, categoryID).embed(["category", "players", "game"] ).exec();
	const actualGameData = gameData.items.game.data;
	const runs = gameData.items.runs.filter(run => run.place.toString() === place.toString()); // TODO run.place === place and have place just get the person in nth place
	const getPlayer = player => gameData.items.players.data.filter(pl => pl.id === player)[0];
	const run_ = runs[0];
	...
	const assetIcon = actualGameData.assets[`trophy-${["1st", "2nd", "3rd", "4th"][run_.place-1]}`];
...
router.add("speedrun rules", [], async(cmd, info) => {
	...
	const gameData = await sr.leaderboards(gameID, categoryID).embed(["category"] ).exec();
