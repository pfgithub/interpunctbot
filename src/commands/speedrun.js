...
async function getURL(strings, ...values) {
	...
	strings.forEach((str, i) => {
		res += str;
		if(values[i]) res += encodeURIComponent(values[i]);
	});
	return JSON.parse((await request(res)).body);
}

async function getGamesFrom({abbreviation}) {
	let gameData = await getURL`https://www.speedrun.com/api/v1/games?abbreviation=${abbreviation}&embed=categories`;
...
async function getCategoriesFromGameID(id) {
	...
	let categoriesGetter = sr.games(id);
...
function parseAbbreviation({from: speedrunpage = ""}) { // I've been doing too much swift
	let match = speedrunpage.match(/[A-Za-z0-9_]+$/); // match the last a-za-z0-9+ of the page. if this doesn't work for all pages, submit a bug report
...
router.add("wr", [], async(cmd, info) => { // wr <category> gets the wr for the specified category||default
	await info.startLoading();
	let [...categoryName] = cmd.split` `;
	categoryName = categoryName.join` `;

	let defaultGameCategory = await info.db.getSpeedrunDefault();
	if(!defaultGameCategory) return await info.error(`Speedrun commands have not been configured for this server.`);
	let {gameID, categoryID} = defaultGameCategory;

	if(categoryName) {
		// wouuldn't it be cool if loading could be updated to say "Getting Category..."
		let categories = await getCategoriesFromGameID(gameID);

		let categoryFilter = categories.items.filter(cat => cat.name.toLowerCase() === categoryName.toLowerCase());
		if(categoryFilter.length <= 0) return await info.error((new MB.TextBuilder).tag`The category \`${categoryName}\` is not in the game.
		Valid categories: ${categories.items.map(cat => cat.name).join`, `}`.build(false)); // TODO return as a mb fields[categoryname,url,name,url...]
		categoryID = categoryFilter[0].id;
	}

	let place = 1;

	let gameData = await sr.leaderboards(gameID, categoryID).embed(["category", "players", "game"] ).exec();
	let actualGameData = gameData.items.game.data;
	let runs = gameData.items.runs.filter(run => run.place.toString() === place.toString()); // TODO run.place === place and have place just get the person in nth place
	let getPlayer = player => gameData.items.players.data.filter(pl => pl.id === player)[0];
	let run_ = runs[0];
	if(!run_) return await info.error("No world record found");
	let run = run_.run;
	...
	mb.description.tag`[${duration.format("y [years] M [months] w [weeks] d [days,] h[h]:mm[m]:s.SSS[s]")}](`;
...
router.add("speedrun leaderboard", [], async(cmd, info) => { // speedrun leaderboard <<abbreviation> category> n gets the person in nth placed
	let [position, ...categoryName] = cmd.split` `;
	categoryName = categoryName.join` `;

	if(!position || !isNormalInteger(position)) return info.error(`A position is required, such as \`speedrun leaderboard 26\``);
...
router.add("speedrun rules", [], async(cmd, info) => {
	...
	let gameData = await sr.leaderboards(gameID, categoryID).embed(["category"] ).exec();
...
async function getGameAtPage(abbreviation) {
	...
	let games = (await getGamesFrom({abbreviation: abbreviation})).data;
	if(games.length < 1) return ((new MB.TextBuilder).tag`No games found for the abbreviation \`${abbreviation}\`. Is the URL right?`.build(false)); // a textbuilder is used here because what if parseabbreviation is updated to allow @
	if(games.length > 1) return ((new MB.TextBuilder).tag`Too many games were found for the abbreviation \`${abbreviation}\`. This should never happen, submit a bug report on the support server (\`about\`) or gitlab page`.build(false));
...
adminrouter.add("speedrun set", [r.manageBot], async(cmd, info) => {
	...
	let abbreviation = parseAbbreviation({from: speedrunpage});
	if(!abbreviation || !categoryName) return await info.error(`Usage: \`speedrun set https://speedrun.com/mygame My Category\``);
	await info.startLoading();
	let game = await getGameAtPage(abbreviation);
	if(typeof game === "string") return await info.error(game);
	...
	if(!category) return await info.error((new MB.TextBuilder).tag`The category \`${categoryName}\` is not in the game.
	...
	// success
	return await info.success((new MB.TextBuilder).tag`Default speedrun page updated to ${abbreviation}: ${categoryName} (ids: ${gameID}, ${category.id})`.build(false));
