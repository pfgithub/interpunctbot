...
module.exports = (type) => new Usage({
	description: `Get a ${type} from the list`,
	usage: ["single?", "search string...?", "number?"],
	callback:	async(data, ...searchString) => {
		let pastebinId = data.allPastebin[type];
		if(!pastebinId) return await data.msg.reply("List not found");
